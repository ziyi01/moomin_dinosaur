...
void print_sieves(int max){
    ...
    //create array located on the stack
	int prime[max+1];
    ...
    //stopping at sqaure root of max, since enough numbers should have been removed by then
    //making it unneccessary to continue.
    //Numbers considered to be primes are marked 0, non-primes 1.
	for(int i = 2; i*i <= max; i++)
	{
        //if number hasn't been marked as non-prime yet then assume it is a prime
		if(prime[i] == 0)
		{
